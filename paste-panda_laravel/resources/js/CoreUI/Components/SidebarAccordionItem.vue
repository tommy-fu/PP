<template>
	<div class="accordion-view-item-wrapper" style="cursor: pointer;">
		<div class="accordion-view-item" @click="expanded = !expanded">
			<div class="title-label-wrapper">
				<div class="body-lg mr-12" :class="{'color-fff': expanded, 'color-a3a3a3' : !expanded}">{{title}}</div>
				<Badge :label="label"/>
			</div>
			<SvgIcon icon="arrow-down-active" :size="16" stroke="#FFFFFF" v-if="expanded" style="font-size: 8px;" class="icon-arrow-down-active"></SvgIcon>
			<SvgIcon icon="arrow-right" :size="16" stroke="#777777" style="font-size: 8px;" class="icon-arrow-right" v-else></SvgIcon>
		</div>
		<div class="accordion-items" v-if="expanded">
			<slot></slot>
		</div>
	</div>
</template>
...
<script>
    ...
    export default {
        name: "sidebar-accordion-item",
        components: {SvgIcon, Badge},
        props: {
            title: String,
		    label: Boolean
