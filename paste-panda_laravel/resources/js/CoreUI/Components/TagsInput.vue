  ...
  <div class="tags-input-root" style="position: relative">
    ...
    <div v-show="searchResults.length">
      ...
      >
        ...
        <li
          style="width: 100%; display:flex; height: 56px; align-items:center; justify-content:space-between;"
          v-for="(tag, index) in searchResults"
          :key="index"
          @mouseover="searchSelection = index"
          @mousedown.prevent="tagFromSearchOnClick(tag)"
        ...
        <div  style="padding: 4px 8px; margin-right: 32px;">{{categoryName}}</div>
        <div style="padding: 4px 8px;border: 1px solid #4e5152;border-radius: 6px;font-family: Inter, sans-serif; font-size: 14px;">{{tag.value}}</div>
        </div>
        <div style="padding: 4px 8px; font-size: 24px">+</div>
        </li>
      </ul>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    hiddenInputValue(tag) {
      ...
      if (fields.length === 1) {
        ...
        return JSON.stringify(
          Object.assign({}, ...fields.map((field) => ({ [field]: tag[field] })))
        );
      }
    ...
    getCategoryField(tag) {
      ...
      return this.categoryName + ' ' +  hasDisplayField ? tag[this.displayField] : tag[this.textField];
    },
    cloneArray(arr) {
